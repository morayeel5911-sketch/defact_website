...
def optimize_file(local_path: Path, quality: int = 85) -> Optional[Path]:
    """Convert PNG/JPG to WebP using cwebp, Pillow, or sips. Returns path to webp if created."""
    if not str(local_path).lower().endswith((".png", ".jpg", ".jpeg")):
        return None
    webp_path = local_path.with_suffix(".webp")

    # 1. cwebp CLI (best quality/size ratio, preferred)
    cwebp = shutil.which("cwebp")
    if cwebp:
        cmd = [cwebp, "-q", str(quality), "-alpha_q", "100", str(local_path), "-o", str(webp_path)]
        result = subprocess.run(cmd, capture_output=True)
        if result.returncode == 0 and webp_path.exists():
            print(f"  [opt] Created {webp_path.name} (cwebp)")
            return webp_path
    ...
    try:
        ...
        if webp_path.exists():
            print(f"  [opt] Created {webp_path.name} (Pillow)")
            return webp_path
    except Exception:
        pass

    # 3. sips fallback on macOS
